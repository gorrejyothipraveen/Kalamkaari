...
function StockBadge({ qty }: { qty: number }) {
  if (qty === 0)
    return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-destructive/10 text-destructive">Out of stock</span>;
...
export function InventoryPage() {
  ...
  const SortIcon = sortDir === "stock_asc" ? ArrowUp : sortDir === "stock_desc" ? ArrowDown : ArrowUpDown;
                ...
                <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {editingId === p.id ? (
                      ...
                      <span className="font-mono">{p.stockQuantity}</span>
                    )}
                  </td>
